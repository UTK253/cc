...
import { db, Appointment, User } from "@/lib/db";
import { Button } from "@/components/ui/button";
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";
import {
    Calendar, Smile, BookOpen, Clock, Settings, LogOut,
    Video, MessageCircle, User as UserIcon, AlertTriangle
} from "lucide-react";
...
export default function DashboardPage() {
    ...
    const fetchData = useCallback(() => {
        ...
        const data = db.getStudentData(currentUser.id);
    ...
    const handleCancel = () => {
        if (cancelModal) {
            db.updateAppointmentStatus(cancelModal.id, "Cancelled");
            setCancelModal(null);
        }
    };
                                        ...
                                        <div key={apt.id} className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 border rounded-xl bg-muted/30 gap-4">
                                            ...
                                            <div className="flex items-center gap-2 w-full sm:w-auto">
                                                ...
                                                <Button size="sm" asChild>
                                                    <Link href={`/session/${apt.id}`}>Join Session</Link>
                                                </Button>
                                            </div>
                                        </div>
                                    );
